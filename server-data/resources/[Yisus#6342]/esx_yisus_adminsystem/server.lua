...
RegisterCommand("tpm", function(source, args, rawCommand)	-- /tpm		teleport to waypoint
	if source ~= 0 then
		...
		if havePermission(xPlayer) then
			xPlayer.triggerEvent("esx_yisus_adminsystem:tpm")
		end
	end
end, false)
...
RegisterCommand("noclip", function(source, args, rawCommand)	-- /goback will teleport you back where you was befor /goto
	if source ~= 0 then
	  	local xPlayer = ESX.GetPlayerFromId(source)
	  	if havePermission(xPlayer) then
	    	xPlayer.triggerEvent("esx_yisus_adminsystem:noclip")
	  	end
	end
end, false)
---------- kill ----------
		...
		if havePermission(xPlayer) then
			if args[1] and tonumber(args[1]) then
				local targetId = tonumber(args[1])
      			local xTarget = ESX.GetPlayerFromId(targetId)
      			if xTarget then
					xTarget.triggerEvent("esx_yisus_adminsystem:killPlayer")
	...
	if source ~= 0 then
  		...
  		if havePermission(xPlayer) then
    		if args[1] and tonumber(args[1]) then
      			...
      			local xTarget = ESX.GetPlayerFromId(targetId)
      			if xTarget then
        			xTarget.triggerEvent("esx_yisus_adminsystem:freezePlayer", 'freeze')
	...
	if source ~= 0 then
  		...
  		if havePermission(xPlayer) then
    		if args[1] and tonumber(args[1]) then
      			...
      			local xTarget = ESX.GetPlayerFromId(targetId)
      			if xTarget then
        			xTarget.triggerEvent("esx_yisus_adminsystem:freezePlayer", 'unfreeze')
...
RegisterCommand("staff", function(source, args, rawCommand)	-- /a command for adminchat
	if source ~= 0 then
		local xPlayer = ESX.GetPlayerFromId(source)
		if havePermission(xPlayer) then
			if args[1] then
