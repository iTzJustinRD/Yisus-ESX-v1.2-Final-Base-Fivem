...
window.onload = function () {
		...
		var eventCallback = {
			ui: function(data) {
				...
				if (config.showDrug == true) { document.querySelector('#drug').style.display = 'block'; }
				if (config.showDirty == true) { document.querySelector('#dirty').style.display = 'block'; }
			...
			setStatus: function(data) {
				...
				if (data.isdead == true) {
					...
					for (i = 0; i < data.status.length; i++) {
						if ((data.status[i].name == 'hunger') || (data.status[i].name == 'thirst') || (data.status[i].name == 'stress') || (data.status[i].name == 'drunk') || (data.status[i].name == 'drug') || (data.status[i].name == 'dirty')) { var statusValue = Math.floor(100 - data.status[i].value); }
