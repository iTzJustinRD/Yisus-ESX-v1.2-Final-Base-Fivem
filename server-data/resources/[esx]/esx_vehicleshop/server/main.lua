...
Citizen.CreateThread(function()
	local char = Config.PlateNumbers
	char = char + Config.PlateLetters + Config.PlateNumbers2
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function(source, cb, model, plate)
	...
	if xPlayer.getAccount('bank').money < modelPrice then
		cb(false)
	else
...
ESX.RegisterServerCallback('esx_vehicleshop:giveBackVehicle', function(source, cb, plate)
	...
	MySQL.Async.fetchAll('SELECT vehicle FROM owned_vehicles WHERE owner = @identity', {
		['@identity'] = ident
	}, function(result)
		if result[1] then
