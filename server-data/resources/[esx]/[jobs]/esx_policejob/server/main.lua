...
AddEventHandler('esx_policejob:putStockItems', function(itemName, count, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	elseif type == 'item_account' then
		...
		if playerAccountMoney >= count and count > 0 then
			...
			xPlayer.showNotification('Has quemado '..count..'$ de dinero negro')
		else
...
ESX.RegisterServerCallback('esx_policejob:getOtherPlayerData', function(source, cb, target, notify)
	...
	if xPlayer then
		...
		--print(target, xPlayer.job.label, data.name, data.job, data.grade, data.inventory, data.accounts, data.weapons)
