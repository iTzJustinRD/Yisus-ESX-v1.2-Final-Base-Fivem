...
local isDead, isBusy puedoFixkit = false, false, false
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint2.Pos, 315.0, function(vehicle)
							ESX.Game.SetVehicleProperties(vehicle, vehicleProps)
							local playerPed = PlayerPedId()
							TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
							TriggerEvent('dps_citizen:DarLlave', vehicle)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
					...
					if Config.MaxInService == -1 then
						ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint2.Pos, 315.0, function(vehicle)
							local playerPed = PlayerPedId()
							TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
							TriggerEvent('dps_citizen:DarLlave', vehicle)
						end)
...
function OpenMobileMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		elseif data.current.value == 'jobnpc' then
			if NPCOnJob then
			...
			Citizen.CreateThread(function()
				while currentTask.busy do
					...
					if not DoesEntityExist(vehicle) and currentTask.busy then
						ESX.ShowNotification('Te has alejado mucho del veh�culo')
...
function MecanicosConnected(connectedPlayers)
	local mechanic = 0
	...
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
				...
				elseif CurrentAction == 'delete_vehicle2' then
		...
		if IsControlJustReleased(0, 38) and currentTask.busy then
			ESX.ShowNotification('Incautaci�n cancelada')
			ESX.ClearTimeout(currentTask.task)
