	...
	for k,v in pairs(Config.Menus) do
		...
		if data.value == k then
			...
			if v.modType then
				...
				elseif v.modType == 'color1' or v.modType == 'color2' or v.modType == 'pearlescentColor' or v.modType == 'wheelColor' or v.modType == 'xenonColor' then
				...
				elseif v.modType == 'xenonColor' then
					for _, xcolor in pairs(Config.XenonColors) do
				...
				elseif v.modType == 48 then -- Livery
					...
					for j = 1, modCount, 1 do
						if j == currentMods[k] then
							_label = ('stickers') .. ' ' .. j .. ' - <span style="color:cornflowerblue;">'.. _U('installed') ..'</span>'
						else
							price = math.floor(vehiclePrice * v.price / 100)
