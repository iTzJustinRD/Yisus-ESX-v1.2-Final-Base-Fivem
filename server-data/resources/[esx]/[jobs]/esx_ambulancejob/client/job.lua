...
function OpenAmbulanceActionsMenu()
	local elements = {}
	table.insert(elements, {label = _U('cloakroom'), value = 'cloakroom'})
	table.insert(elements, {label = 'Depositar objetos',  value = 'put_stock'})
	table.insert(elements, {label = 'Sacar objetos', value = 'get_stock'})
			...
			if data.current.type == 'item_weapon' then
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
					...
					if count == nil then
						...
						TriggerServerEvent('esx_ambulancejob:putStockItems', data.current.type, data.current.value, count)
						menu2.close()
						menu.close()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_ambulancejob:showStockItems', function(inventory)
		local elements = {}
		local menutitle = "Armario"
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			if data.current.type == 'item_weapon' then
				...
				TriggerServerEvent('esx_ambulancejob:getStockItem', data.current.type, data.current.value, data.current.ammo)
				ESX.SetTimeout(300, function()
					OpenGetStocksMenu()
				end)
			else
...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy', {
		...
		elements = {
			...
			{label = _U('pharmacy_take', 'Ibuprofeno'), item = 'medicpill', type = 'slider', value = 1, min = 1, max = 100},
