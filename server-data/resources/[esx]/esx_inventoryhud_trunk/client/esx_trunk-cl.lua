...
function VehicleInFront()
  ...
  if GetEntityPopulationType(result) == 7 then
    return result
  end
  return false
end
...
function openmenuvehicle()
  ...
  if vehicle == false then
    ESX.ShowNotification("Ningún coche con maletero cerca")
    return
  end
  ...
  if not IsPedInAnyVehicle(playerPed) then
    ...
    if not Config.CheckOwnership or (Config.AllowPolice and ESX.PlayerData.job.name == "police") or (Config.CheckOwnership and myVeh) then
      if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
        ...
        if vehFront ~= false and vehFront ~= 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
