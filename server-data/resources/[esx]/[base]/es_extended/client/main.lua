...
AddEventHandler('esx:onPlayerSpawn', function() isDead = false TriggerServerEvent('crew:onPlayerLoaded', GetPlayerServerId(PlayerId())) end)
...
--Afk kick
local afkTimeout = Config.afkTime 
local timer = 0
			...
			if playerPed then
				...
				if currentPosition == previousPosition and currentHeading == previousHeading then
					if timer > 0 then
						...
						TriggerServerEvent('es_extended:afkKick')
					end
				else
					timer = afkTimeout
				end
