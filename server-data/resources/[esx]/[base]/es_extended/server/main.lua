...
function onPlayerJoined(playerId)
	...
	if steamhex then
		MySQL.Async.fetchScalar('SELECT 1 FROM users WHERE identifier = @identifier', {
			...
			if result then
				MySQL.Async.execute('UPDATE users SET steamhex = @steamhex WHERE identifier = @identifier', {
					['@steamhex'] = steamhex,
					['@identifier'] = identifier
				}, function(rowsChanged)
				end)
		...
		print("Ha ocurrido un error al guardar el SteamHEX de "..playerId)
	end
end
...
--AFK KICK
RegisterServerEvent('es_extended:afkKick')
AddEventHandler('es_extended:afkKick', function()
	DropPlayer(source, ('Has sido expulsado por estár AFK.'))
end)